...
<?php
	...
	if(isset($_GET['total'])){
		...
		$results = $mydb->query($sql);
		if(!$results){
			echo "No daily report history yet!<br>";
		}
		$total = $results[0][0];
	}
	if(isset($_GET['n'])){
	...
	if(isset($_GET['from'])){
		...
		$condition= $condition . " and daily_date >= '{$from}' ";
	}
	if(isset($_GET['to'])){
		$to = $_GET['to'];
		$to = str_replace('-', '', $to);
		$condition=$condition . " and daily_date <='{$to}' ";

	}
	if(isset($_GET['status'])){
		$status = $_GET['status'];

		$condition =$condition . " and daily_status = {$status} ";

	}
	//echo $condition;
	$sql = "select * from (select * from tt_daily_report where author_id = {$userID} {$condition}) as tempTable limit {$temp}, {$tempLimit} ";
	$results = $mydb->query($sql);
	if(!$results){
		echo "No daily report history yet!<br>";
	}
...
tableRaw;
		...
		for($i=0;$i<=($total-1)/PAGE_NUM;$i++){
			$page = $i+1;
			echo "<a href='./dailyReportHistory.php?total={$total}&n={$i}'>{$page}</a>&nbsp";
		}
		echo "</td></tr>";
	}
	?>
