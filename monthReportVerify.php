...
<?php
	...
	if(isset($_GET['total'])){
		...
		$sql = "select count(*) from tt_month_report where month_status = 0 and author_id in (select user_id from tt_user where leader_id = {$leaderID})";
		$results = $mydb->query($sql);
		if(!$results){
	...
	$temp = $page*PAGE_NUM;
	$tempLimit = PAGE_NUM;
	$sql = "select * from (select * from tt_month_report where month_status = 0 and author_id in (select user_id from tt_user where leader_id = {$leaderID})) as temptable limit {$temp}, {$tempLimit}";

	$results = $mydb->query($sql);
	if(!$results){
		echo "No submitted month report yet !<br>";
	}
	else{
		for($i=0;$i<count($results);$i++){
			...
			<tr>
				...
				<a href="javascript:void(0)" onclick="verify('./reportOp.php?type=1&status=1&id={$temp[DAILY_ID]}')">Verify</a>&nbsp
				<a href="javascript:void(0)" onclick="verify('./reportOp.php?type=1&status=2&id={$temp[DAILY_ID]}')">Deny</a>&nbsp
				</td>
			</tr>
tableRaw;
		...
		for($i=0;$i<=($total-1)/PAGE_NUM;$i++){
			$page = $i+1;
			echo "<a href='./monthReportVerify.php?total={$total}&n={$i}'>{$page}</a>&nbsp";
		}
		echo "</td></tr>";
